...
static bool enclave_mode = false; //cch added
                           ...
                           int error_code, target_ulong next_eip, int is_hw)
{
    ...
    //fprintf(stderr, "cch debug intno: %d, is_int: %d, next_eip: %p, is_hw: %d, env->eip in do_interrupt64\n", intno, is_int, next_eip, is_hw, env->eip);
    ...
    PUSHQ(esp, old_eip);
    //fprintf(stderr, "cch debug pushed esp: %p, pushed eip: %p in do_interrupt64\n", env->regs[R_ESP], old_eip);
    if (has_error_code) {
    ...
    cpu_x86_load_seg_cache(env, R_CS, selector,
                   ...
                   e2);
    env->eip = offset;
    //fprintf(stderr, "cch debug changed esp: %p, changed eip: %p in do_interrupt64\n", env->regs[R_ESP], env->eip);
}
#endif
                             ...
                             int error_code, target_ulong next_eip, int is_hw)
{
    ...
    if (env->cr[0] & CR0_PE_MASK) {
        ...
        if (env->hflags & HF_LMA_MASK) {
            /*
            enclave_mode = env->cregs.CR_ENCLAVE_MODE; //cch added
            if (enclave_mode) { //cch added
                 helper_sgx_ehandle(env, intno, is_hw); //cch: is is_hw equal to is_int? I need to check this again
            }
            */
...
void x86_cpu_do_interrupt(CPUState *cs)
{
    ...
    if (env->cregs.CR_ENCLAVE_MODE) {
         helper_sgx_ehandle(env, cs->exception_index, env->exception_is_int); 
    }
    if (env->cregs.CR_IS_IN_ERESUME) { //cch temporarily added
         fprintf(stderr, "Fault happens while handling ERESUME. EIP was at %"PRIx64"\n", env->eip);
         //cch: do I need to set EIP as AEP so that exception handler can return to this point -> yes
         env->eip = env->cregs.CR_AEP;
         fprintf(stderr, "Now EIP is changed to %"PRIx64". This will be pushed on the stack\n", env->eip);
    }
                                        ...
                                        int is_iret, int addend)
{
    ...
    //printf("cch new eip is %p in helper_ret_protected\n", new_eip);
