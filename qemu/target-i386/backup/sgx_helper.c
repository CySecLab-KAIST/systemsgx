...
//cch added
uint64_t return_hostaddr(CPUX86State *env, uint64_t guestaddr){ //cch added
    int page_index;
    int mmu_idx;
    int temp;
    int size = 4; //cch temporary
    page_index = (guestaddr >> TARGET_PAGE_BITS) & (CPU_TLB_SIZE - 1);
    mmu_idx = cpu_mmu_index(env);//CPU_MMU_INDEX; 
    uint64_t hostaddr;

    if (unlikely(env->tlb_table[mmu_idx][page_index].addr_read !=
                 (guestaddr & (TARGET_PAGE_MASK | (size - 1))))) {    //cch: modified DATA_SIZE to size and ADDR_READ to addr_read. But, I am not sure whether they are correct
       //sgx_dbg(trace, "tlb_table is not matched with guestaddr & page_mask");
       temp = glue(glue(helper_ld, q), _mmu)(env, guestaddr, mmu_idx); //cch: modified SUFFIX to q and MMUSUFFIX to _mmu, I am not sure whether they are correct
       hostaddr = guestaddr + env->tlb_table[mmu_idx][page_index].addend; //cch added
       //res = ldx_raw(hostaddr, size); //cch added
    }   
    else{
       hostaddr = guestaddr + env->tlb_table[mmu_idx][page_index].addend;
       //res = ldx_raw(hostaddr, size); //cch modified
    }   

    return hostaddr;
}

// cch added. Check if mem_addr is in EPC
static
bool linaddr_is_within_epc(CPUX86State *env, uint64_t mem_addr)
{
    sgx_dbg(trace, "linaddr_is_within_epc entry point mem_addr: %p", mem_addr);
    int16_t index = -1;
    int i;
    uint64_t hostaddr = return_hostaddr(env, mem_addr);

    sgx_dbg(trace, "guestaddr is %p, hostaddr is %p", mem_addr, hostaddr);
    for (i = 0; i < NUM_EPC; i ++) {
    ...
    if (index == -1) {
        sgx_msg(warn, "linaddr is not within epc");
        return false;
    }
...
// cch added. InEPC check
static
void check_linaddr_within_epc(void *page_addr, CPUX86State *env)
{
    sgx_dbg(trace, "check_linaddr_within_epc entry point");
    assert(page_addr);
    assert(env);
...
void saveState(gprsgx_t* page, CPUX86State *env)
{
    ...
    // cch added
    //gprsgx_t* tmp_page = cpu_ldx_data(env, page, sizeof(gprsgx_t)); //cch: this seems to be correct. Does this make error?
    //gprsgx_t* tmp_page = malloc(sizeof(gprsgx_t));
    ...
    page->rip = env->eip; //cch added
    sgx_dbg(trace, "EIP saved will be %p", (void *)page->rip);
    sgx_dbg(trace, "State is saved in tmp_page");
    // cch added
    //cpu_stx_data(env, page, tmp_page, sizeof(gprsgx_t));
    //sgx_dbg(trace, "State is stored in %p", (void *)page);
    //free(tmp_page);
    //sgx_dbg(trace, "free of tmp_page is successful");
}
...
// cch added: Searches EPCM for linear address
static
uint16_t epcm_linsearch(void *addr, CPUX86State *env)
{
    ...
    for (i = 0; i < NUM_EPC; i ++) {
        // Can be in between page addresses. for example: EEXTEND : 256 chunks && EWB : Version Array (VA)
        if ((epcm[i].epcHostAddress <= hostaddr) //cch modified
                && (hostaddr < epcm[i].epcHostAddress + PAGE_SIZE)) {
            index = i;
            break;
        }        
                              ...
                              int alignment, perm_check_t perm)
{
    ...
    uint16_t index_page = epcm_linsearch(reg, env); //cch modified
...
void sgx_eaccept(CPUX86State *env)
{
    ...
    secinfo_t *scratch_secinfo; //cch modified from secinfo_t scratch_secinfo
    ...
    check_linaddr_within_epc(tmp_secinfo, env); //cch modified

    uint16_t index_secinfo = epcm_linsearch(tmp_secinfo, env); //cch modified
    sgx_dbg(trace, "cch debug index of secinfo: %d", index_secinfo);
    ...
    scratch_secinfo = cpu_ldx_data(env, tmp_secinfo, sizeof(secinfo_t)); //cch modified
    ...
    check_linaddr_within_epc(destPage, env); //cch modified
    ...
    uint16_t index_page = epcm_linsearch(destPage, env); //cch modified
    sgx_dbg(trace, "cch debug index of destPage: %d", index_page);
    epcm_entry_t *epcm_dest = &epcm[index_page];

    sgx_dbg(trace, "cch debug valid:%d, blocked:%d, page_type:%d, enclave_secs: %x, CR_ACTIVE_SECS: %x", epcm_dest->valid, epcm_dest->blocked, epcm_dest->page_type, epcm_dest->enclave_secs, env->cregs.CR_ACTIVE_SECS);
    // TODO: PT_TRIM needs to be added
    ...
    int new_cpl = 0; 
    env->hflags = (env->hflags & ~HF_CPL_MASK) | new_cpl;
    sgx_dbg(trace, "cpl in eaug is now changed from %d to %d", old_cpl, new_cpl);

    secs_t *tmp_secs_host = cpu_load_secs(env, tmp_secs); //cch added

    old_cpl = env->hflags & HF_CPL_MASK; 
    new_cpl = 3; 
    env->hflags = (env->hflags & ~HF_CPL_MASK) | new_cpl; 
    sgx_dbg(trace, "cpl in eaug is now changed from %d to %d", old_cpl, new_cpl);
    ...
    if (scratch_secinfo->flags.page_type == PT_TCS){
        ...
        tcs_t *tcs_host = (tcs_t *)cpu_ldx_data(env, tcs, sizeof(tcs_t)); //cch added
...
void sgx_eenter(CPUX86State *env)
{
    ...
    tcs_t tmp_tcs;  // cch added
    cpu_load_obj(env, &tmp_tcs, tcs, sizeof(tcs_t));  // cch added

    index_tcs = epcm_linsearch(tcs, env); //cch modified
    ...
    secs_t *secs =  get_secs_address(&epcm[index_tcs]); // TODO: Change when the ENCLS is implemented - pageinfo_t   cch: change tmp_secs to secs
       
    int old_cpl = env->hflags & HF_CPL_MASK; 
    int new_cpl = 0; 
    env->hflags = (env->hflags & ~HF_CPL_MASK) | new_cpl;
    sgx_dbg(trace, "cpl in eenter is now changed from %d to %d", old_cpl, new_cpl);

    secs_t *tmp_secs = cpu_load_secs(env, secs); // cch: added, need to be freed later
    // XXX: unused
...
// TODO: Implement XSAVE/XSTOR related spec
    ...
    index_gpr = epcm_linsearch((void *)tmp_gpr, env);  //cch modified from epcm_search

    // Temporarily block
    check_linaddr_within_epc((void *)tmp_gpr, env); //cch modified from check_within_epc
    ...
    */
    ...
    env->cregs.CR_ACTIVE_SECS = (uint64_t)secs; //cch modified
    ...
    sgx_dbg(trace, "CR_AEP: %p", env->cregs.CR_AEP); //cch added for debug
...
*/
    ...
    gprsgx_t *tmp_gpr_host; //cch added , name might be changed later
    tmp_gpr = (gprsgx_t *)env->cregs.CR_GPR_PA; //cch added
    ...
    qenclaves[eid].stat.enclu_n++;
#endif
    //cch: should add a code for storing from tmp_gpr to gpr
...
void sgx_eexit(CPUX86State *env)
{
    ...
    gprsgx_t *tmp_gpr;
    tcs_t *tcs;
    secs_t *tmp_secs_host; //cch added
    gprsgx_t *tmp_gpr_host; //cch added , name might be changed later
    tcs_t *tmp_tcs_host; //cch added

    //cch: should I escalate privilege here?
    /* cch added */
    int old_cpl = env->hflags & HF_CPL_MASK;
    int new_cpl = 0;
    env->hflags = (env->hflags & ~HF_CPL_MASK) | new_cpl;
    sgx_dbg(trace, "cpl in eexit is now changed from %d to %d", old_cpl, new_cpl);

    sgx_dbg(trace, "Current ESP: %p   EBP: %p", (void *)env->regs[R_ESP], (void *)env->regs[R_EBP]);
    sgx_dbg(trace, "CR_ACTIVE_SECS: %p", (void *)env->cregs.CR_ACTIVE_SECS); //cch added
    ...
    if(retAddr != 0) {
        ...
        tmp_tcs_host = cpu_ldx_data(env, tcs, sizeof(tcs_t)); //cch added, should be freed later. should be copied to original
        {
        ...
        tmp_gpr_host = cpu_ldx_data(env, tmp_gpr, sizeof(gprsgx_t)); //cch: tmp_gpr should be copied to original. should be freed later
        ...
        cpu_stx_data(env, tcs, tmp_tcs_host, sizeof(tcs_t)); //cch added
        free(tmp_tcs_host); //cch added
    }
    else {
        sgx_dbg(trace, "cch: entry of else. tmp_gpr will be loaded");
        tmp_gpr = (gprsgx_t *)env->cregs.CR_GPR_PA; //cch added
        tmp_gpr_host = cpu_ldx_data(env, tmp_gpr, sizeof(gprsgx_t)); //cch: tmp_gpr don't have to be copied to original, but should be freed later
        sgx_dbg(trace, "cch: loading tmp_gpr passed");
    ...
    env->regs[R_EBX] = env->cregs.CR_TCS_LA; //cch added for multi-threading (no SPEC)
    ...
    tmp_secs_host = cpu_load_secs(env, secs); //cch added
    if (!removeEnclaveEntry(tmp_secs_host)) {  //cch modified
        //raise_exception(env, EXCP0D_GPF);
    }
...
//    setEnclaveAccess(false);
    ...
    qenclaves[eid].stat.enclu_n++;
#endif
    //cch: should add a code for stroing from tmp_gpr to tmp_gpr_host
    cpu_stx_data(env, tmp_gpr, tmp_gpr_host, sizeof(gprsgx_t));
    free(tmp_secs_host); //cch added
    free(tmp_gpr_host); //cch added
    /* cch added */
    old_cpl = env->hflags & HF_CPL_MASK;
    new_cpl = 3;
    env->hflags = (env->hflags & ~HF_CPL_MASK) | new_cpl;
    sgx_dbg(trace, "cpl in eexit is now changed from %d to %d", old_cpl, new_cpl);
    sgx_dbg(trace, "cch: sgx_eexit end");
}
...
void sgx_egetkey_param_check(CPUX86State *env)
{
    ...
    keyrequest_t *tmp_keyrequest_host = cpu_ldx_data(env, tmp_keyrequest, sizeof(keyrequest_t)); //cch added
...
void sgx_egetkey(CPUX86State *env)
{
    ...
    int new_cpl = 0; 
    env->hflags = (env->hflags & ~HF_CPL_MASK) | new_cpl;
    sgx_dbg(trace, "cpl in egetkey is now changed from %d to %d", old_cpl, new_cpl);
    secs_t *tmp_currentsecs_host = cpu_load_secs(env, tmp_currentsecs); 
    old_cpl = env->hflags & HF_CPL_MASK;
    new_cpl = 3; 
    env->hflags = (env->hflags & ~HF_CPL_MASK) | new_cpl;
    sgx_dbg(trace, "cpl in egetkey is now changed from %d to %d", old_cpl, new_cpl);
    ...
    cpu_stx_data(env, (uint8_t *)outputdata, tmp_key, 16); //cch modified from memcpy((uint8_t *)outputdata, tmp_key, 16);
...
void sgx_eresume(CPUX86State *env)
{
    ...
    tcs_host = cpu_ldx_data(env, tcs, sizeof(tcs_t)); //cch added, should be freed later. should be copied to original
...
*/
    ...
    // Temporarily block
    check_linaddr_within_epc(tcs, env); //cch modified from check_within_epc
    // Check if AEP is canonical
    if (tmp_mode64) {
    ...
    int new_cpl = 0; 
    env->hflags = (env->hflags & ~HF_CPL_MASK) | new_cpl;
    sgx_dbg(trace, "cpl in eresume is now changed from %d to %d", old_cpl, new_cpl);
...
// TODO: Implement XSAVE/XSTOR related spec
    ...
    // Temporarily block
    check_linaddr_within_epc((void *)tmp_gpr, env); //cch modfieid from check_within_epc
    // Check for validity and block
    epcm_invalid_check(&epcm[index_gpr], env);
    epcm_blocked_check(&epcm[index_gpr], env);
...
//    env->cregs.CR_TCS_LA = (uint64_t)tcs_app;
    ...
    */
    ...
    // Restore GPRs
    restoreGPRs((gprsgx_t *)tmp_gpr_host, env); //cch changed
    env->cregs.CR_EXIT_EIP = ((gprsgx_t *)tmp_gpr_host)->SAVED_EXIT_EIP; //cch changed
...
void sgx_ecreate(CPUX86State *env)
{
    ...
    sgx_dbg(trace, "cch debug: cpl in ecreate is %d", cpl);
    ...
    void *tmp_linaddr;
    secs_t *tmp_secs_pi;
    secinfo_flags_t tmp_secinfo_flags; //cch added
    uint64_t *tmp_secinfo_reserved; //cch added
    secs_t *tmp_secs_host; //cch added  
    ...
    if (tmp_secinfo_flags.page_type != PT_SECS) { //cch: changed
        raise_exception(env, EXCP0D_GPF);
    }
    ...
    tmp_secinfo_reserved = cpu_load_si_reserved(env, tmp_secinfo); //cch added
    ...
    if ((tmp_secs_host->attributes.xfrm & 0x3) != 0x3) { //cch modified
        raise_exception(env, EXCP0D_GPF);
    }
...
//      raise_exception(env, EXCP0D_GPF);
    ...
    sgx_dbg(info, "baseAddr: %p, size: %x", tmp_secs_host->baseAddr, tmp_secs_host->size); //cch added for debug

    // ATTRIBUTES MODE64BIT, TMP_SECS SIZE check
    if (tmp_secs_host->attributes.mode64bit == 0) { // cch modified
        checkReservedBits((uint64_t *)&tmp_secs_host->baseAddr,
                          0x0FFFFFFFF00000000L, env); //cch modified
        checkReservedBits((uint64_t *)&tmp_secs_host->size,
                          0x0FFFFFFFF00000000L, env);
    } else { // tmp_secs->attributes.mode64bit == 1
        is_canonical(tmp_secs_host->baseAddr, env); //cch modified
        checkReservedBits((uint64_t *)&tmp_secs_host->size,
...
//        raise_exception(env, EXCP0D_GPF);
    ...
    if ((tmp_secs_host->size < (PAGE_SIZE * 2)) || (popcnt(tmp_secs_host->size) > 1)) {//cch modified
        sgx_dbg(err, "Invalid SECS.SIZE (less than 8192 or not power of 2).");
        raise_exception(env, EXCP0D_GPF);
    }

    // Reserved fields of TMP_SECS must be zero. If not, then GP(0)
    is_reserved_zero(tmp_secs_host->reserved1,  sizeof(((secs_t*)0)->reserved1), env); //cch modified
    is_reserved_zero(tmp_secs_host->reserved2, sizeof(((secs_t*)0)->reserved2), env); //cch modified
    is_reserved_zero(tmp_secs_host->reserved3, sizeof(((secs_t*)0)->reserved3), env); //cch modified
    is_reserved_zero(tmp_secs_host->eid_reserved.reserved,
                     sizeof(((secs_t*)0)->eid_reserved.reserved), env); //cch modified
    ...
    sha256init(tmp_secs_host->mrEnclave); //cch modified

    tmp_secs_host->isvsvn = 0; //cch modified
    tmp_secs_host->isvprodID = 0; //cch modified

    // Initialize enclave's MRENCLAVE update counter
    tmp_secs_host->mrEnclaveUpdateCounter = 0; //cch modified

    // Update MRENCLAVE of SECS
    memset(&tmpUpdateField[0], 0, 64);
    memcpy(&tmpUpdateField[0], &hash_ecreate, sizeof(uint64_t));
    memcpy(&tmpUpdateField[8], &tmp_secs_host->ssaFrameSize, sizeof(uint32_t)); //cch modified
    memcpy(&tmpUpdateField[12], &tmp_secs_host->size, sizeof(uint64_t)); //cch modified
    memset(&tmpUpdateField[20], 0, 44);
    
    // Update MRENCLAVE hash value
    sha256update((unsigned char *)tmpUpdateField, tmp_secs_host->mrEnclave); //cch modified

    // Increase enclave's MRENCLAVE update counter
    tmp_secs_host->mrEnclaveUpdateCounter++; //cch modified


    // Check MRENCLAVE after ECREATE //cch enabled
    {
        char hash[64+1];
        uint64_t counter = tmp_secs_host->mrEnclaveUpdateCounter; //cch modified

        fmt_hash(tmp_secs_host->mrEnclave, hash); //cch modfiied
    ...
    //cch: should add a code for storing from tmp_secs_host to tmp_secs
...
void sgx_eadd(CPUX86State *env)
{
    ...
    //memcpy(&scratch_secinfo, tmp_secinfo, sizeof(secinfo_t)); //cch should modify
    cpu_load_obj(env, &scratch_secinfo, tmp_secinfo, sizeof(secinfo_t)); //cch modfied
    ...
    sgx_dbg(trace, "destPage is %p, tmp_srcpge is %p\n", destPage, tmp_srcpge); //cch added
    cpu_load_n_store(env, destPage, tmp_srcpge, PAGE_SIZE); 
...
*/
    ...
    tmp_secs_host = cpu_load_secs(env, tmp_secs); //cch added 
    tcs_t tcs; //cch modified
    //tcs_t *tcs = NULL;
    ...
    switch(scratch_secinfo.flags.page_type) {
        case PT_TCS: {
            ...
            //tcs = (tcs_t *)tmp_srcpge;  // Needs to be verified cch:seems incorrect
            //tcs = (tcs_t*)pageInfo->srcpge; // Needs to be verified cch:seems incorrect
            ...
            if ((tmp_secs_host->attributes.mode64bit == 0)
                ...
                || ((tcs.gslimit & 0x0FFF) != 0x0FFF))) { //cch modified
                raise_exception(env, EXCP0D_GPF);
            }
            break;
        }
    ...
    if((tmp_linaddr < tmp_secs_host->baseAddr)
        || (tmp_linaddr >= (tmp_secs_host->baseAddr + tmp_secs_host->size))) //cch modified
        raise_exception(env, EXCP0D_GPF);
    ...
    if (scratch_secinfo.flags.page_type == PT_TCS) {
        ...
        tcs.flags.dbgoptin = 0; //cch modified
        tcs.cssa = 0;  //cch modified
        sgx_dbg(trace, "current cssa is %d", tcs.cssa); //cch modified
        cpu_stx_data(env, destPage, &tcs, sizeof(tcs_t)); //cch added. this is not in spec, yet is required to meet the consistency between between host_tmp_address and guest_real_address
    }
    ...
    //epcm[index_page].appAddress = (uint64_t)tmp_srcpge; //cch : this does not seem to be in spec, but from opensgx
...
void sgx_einit(CPUX86State *env)
{
    ...
    secs_t *tmp_secs; //cch added
    
    // set temp variables
    cpu_load_obj(env, &tmp_sig, sig, sizeof(sigstruct_t)); //cch modified
    //memcpy(&tmp_sig, sig, sizeof(sigstruct_t));
    cpu_load_obj(env, &tmp_token, token, sizeof(einittoken_t)); //cch modified
    //memcpy(&tmp_token, token, sizeof(einittoken_t));
    tmp_secs = cpu_load_secs(env, secs);  //cch added
    ...
    // Verify signature
    if (!verify_signature(&tmp_sig, tmp_sig.signature, tmp_sig.modulus, tmp_sig.exponent)) { //cch modified
...
*/
    ...
    //cch: should add a code for storing from tmp_secs(host) to secs(guest)
...
void sgx_eldb(CPUX86State *env)
{
    ...
    uint64_t *tmp_srcpge; //cch changed from epc_t
    ...
    */
    ...
    epcm[epc_index].enclave_secs = tmp_secs;  //cch: this is not in SPEC, but indeed necessary
...
void sgx_eextend(CPUX86State *env)
{
    ...
    // Calculate enclave offset. cch: opensgx is totally different from spec. I would choose spec
    //tmp_enclaveoffset = (uint64_t)target_addr - tmp_secs_host->baseAddr; //from opensgx
    tmp_enclaveoffset = epcm[index_page].enclave_addr - tmp_secs_host->baseAddr; //cch: from spec
    tmp_enclaveoffset = tmp_enclaveoffset + ((uint64_t)target_addr & 0x0FFF); //cch: from spec
    sgx_dbg(eextend, "enclave addr is %p, baseaddr is %p, target_addr is %p, enclave offset is %p", epcm[index_page].enclave_addr, tmp_secs_host->baseAddr, target_addr, tmp_enclaveoffset);  //cch added
    ...
    cpu_load_obj(env, target_addr_host, target_addr, 256); //cch added

    // Add 256 bytes to MRENCLAVE, 64 byte at a time
    sha256update((unsigned char *)(&target_addr_host[0]), tmp_secs_host->mrEnclave); //cch modified
...
void sgx_eaug(CPUX86State *env)
{
    ...
    memset(destPage_host, 0, PAGE_SIZE); //cch modified
    cpu_stx_data(env, destPage, destPage_host, PAGE_SIZE); //cch added
...
// cch: KEY_PATH should be corrected
...
void encls_set_stat(CPUX86State *env)
{
    ...
    cpu_stx_data(env, stat, &(qenclaves[eid].stat), sizeof(stat_t)); // cch modified
}
...
void helper_sgx_ehandle(CPUX86State *env, int intno, int is_int) //cch modified
{
    ...
    int new_cpl = 0; 
    env->hflags = (env->hflags & ~HF_CPL_MASK) | new_cpl;
    sgx_dbg(trace, "cpl in ehandle is now changed from %d to %d", old_cpl, new_cpl);
    ...
    if (!tmp_mode64) {
        ...
        sgx_dbg(info, "saveState is exited in if"); //cch added should be deleted later
        //TODO:    tmp_ssa->rflags.tf = 0;
    } else {
        saveState(tmp_gpr_host, env);
        sgx_dbg(info, "saveState is exited in else"); //cch added should be deleted later
        //TODO:    tmp_ssa->rflags.tf = 0;
    }
    ...
    env->regs[R_ECX] = env->cregs.CR_AEP;
    sgx_dbg(trace, "RAX: %d, RBX: %lx, RCX: %lx", env->regs[R_EAX], env->regs[R_EBX], env->regs[R_ECX]);
    ...
    //CCH: copy tmp_gpr_host to tmp_gpr need to be added
    cpu_stx_data(env, tmp_gpr, tmp_gpr_host, sizeof(gprsgx_t));
