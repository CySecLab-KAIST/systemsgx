...
//#define TIMER_INTERRUPT_CONTROL
#ifdef TIMER_INTERRUPT_CONTROL
    #define TIMER_DELAY 160000000
#endif
#define SGX_DEBUG
#include "sgx-dbg.h"
...
typedef struct {
    ...
    uint64_t enclave_tcs;               // cch TCS identifier of thread to which page belongs
#endif
    ...
    // XXX? cch: the following fields do not exist in spec
    uint64_t epcPageAddress;            // Maps EPCM <-> EPC ( enclaveAddress seems to have a different functionality cch: this field is used for obtaining epcm given the effective address of epc
    uint64_t epcHostAddress;            // cch added for withinEPC check
} epcm_entry_t;
...
#ifdef THREAD_PROTECTION
    uint64_t tcs;                       // Eff. addr. of EPC slot that currently contains a TCS
#endif
} pageinfo_t;
...
typedef struct epc_entry_map {
    uint64_t eid;
    bool active;
    addrRange range;
    lastAddress *lastAddr;
    entry *entry_Map;
    struct epc_entry_map *next;
} epc_map;
...
#ifdef THREAD_PROTECTION
    uint64_t tid;
    uint8_t padding[40]; // padding bytes to make it 128 byte ...
#else
